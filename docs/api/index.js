...
function toggleAutoSave() {
  ...
  if (toggleEl) toggleEl.style.background = autoSaveEnabled ? '#10B981' : '#f59e0b';
  if (autoSaveEnabled) {
...
function downloadHistoryCSV() {
  ...
  history.sort((a,b)=>a.period.localeCompare(b.period)).forEach(d => {
    ...
    csv += `${formatMonth(d.period)},${d.members},${d.contributions},${d.loansDisbursed},${d.loansBalance},${d.totalBankBalance},${d.profit},${d.roa},"${formatDateTime(d.dateSaved)}",${saveTypeLabel},"${bankBreakdown}",${cumLoans},${bookValue}\n`;
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Auto-detect changes every 2 seconds
  setInterval(() => {
    initCarouselData();
    if (window.saccoData?.today) {
      const currentHash = computeDataHash(window.saccoData.today);
...
window.loadLiveHistory = loadLiveHistory;<script>
  const API_BASE = 'https://soyosoyo-sacco-api.onrender.com/api';
  window.saccoHistory = {};
  window.saccoData = window.saccoData || {};
  let retryInterval = null;
  ...
  // Compute data hash for change detection (includes new fields)
  function computeDataHash(today) {
    if (!today) return '';
    return JSON.stringify({
      members: today.members,
    ...
    try {
      ...
      data.forEach(row => {
        window.saccoHistory[row.period] = {
          ...
          members: Number(row.members || 0),
          contributions: Number(row.contributions || 0),
          loansDisbursed: Number(row.loansDisbursed || row.loans || 0), // Compatibility: map old 'loans' to 'loansDisbursed'
          loansBalance: Number(row.loansBalance || 0),
          totalBankBalance: Number(row.totalBankBalance || row.bankBalance || 0), // Compatibility: map old 'bankBalance' to 'totalBankBalance'
          profit: Number(row.profit || 0),
          roa: Number(row.roa || 0),
          extraFields: row.extraFields ? JSON.parse(row.extraFields) : {},
          dateSaved: row.dateSaved,
      ...
      // Ensure current month is shown and updated from carousel/local
      initCarouselData();
      if (window.saccoData?.today) {
        const currentHash = computeDataHash(window.saccoData.today);
        const isDataChanged = currentHash !== lastDataHash;
  ...
  // Initialize data from carousel.js or localStorage
  function initCarouselData() {
    if (!window.saccoData?.today) {
      const savedData = localStorage.getItem('saccoDataToday');
      if (savedData) {
    ...
    // Trigger carousel update if available
    if (typeof window.updateCarouselData === 'function') {
      window.updateCarouselData();
    }
  }
  ...
  async function saveCurrentMonth(auto = false) {
    ...
    const payload = {
      members: today.members,
      contributions: today.contributions,
      loansDisbursed: today.loansDisbursed || today.loans || 0, // Compatibility
      loansBalance: today.loansBalance || 0,
      total_bank_balance: today.totalBankBalance || today.bankBalance || 0, // Compatibility
      profit: today.profit || 0,
      roa: parseFloat(today.roa) || 0,
      extraFields: JSON.stringify(today.extraFields || {})
    };
