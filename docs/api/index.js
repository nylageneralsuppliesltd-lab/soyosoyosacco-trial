...
for (const file of [DB_FILE,PENDING_FILE]) if(!fs.existsSync(file)) fs.writeFileSync(file,"[]","utf8");
...
function saveJSON(file,data){ try{ fs.writeFileSync(file,JSON.stringify(data,null,2)); return true;} catch(e){ return false;} }
...
async function saveToDB(entry){
  ...
  `;
  const values = [
    entry.period, entry.members, entry.contributions, entry.loans_disbursed, entry.loans_balance,
    entry.total_bank_balance, entry.coop_bank, entry.chama_soft, entry.cytonn,
    entry.total_assets, entry.profit, entry.roa, entry.date_saved, JSON.stringify(entry.extra_fields)
  ];
...
app.post("/api/zapier", async (req,res)=>{
  ...
  try{
    // Forward exactly the payload received from frontend
...
cron.schedule("0 9 1 * *", ()=>{ console.log("Running scheduled monthly email..."); sendMonthlyEmail(); });
