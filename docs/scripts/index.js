// scripts/index.js - FULL HISTORY + AUTO-SAVE + SOCIAL POSTING + LOCALSTORAGE FALLBACK
...
function renderFullHistory() {
    ...
    if(tbody){
      tbody.innerHTML=history.map(row=>{
        ...
        const metricsHtml=`
          ...
          <div class="saved-on"><span class="metric-label">ROA:</span> <span class="roa-value">${Number(row.roa).toFixed(1)}%</span> | Saved: ${formatDisplayDate(row.dateSaved)} ${saveTypeLabel}</div>
        `;
  ...
  footer.innerHTML=`<strong>${history.length}</strong> months saved 
...
function initCarouselData(){
  if(!window.saccoData?.today){
    ...
    window.saccoData.today=saved?JSON.parse(saved):{members:0,contributions:0,loansDisbursed:0,loansBalance:0,totalBankBalance:0,profit:0,roa:0,extraFields:{}};
  }
...
async function attemptSave(payload,saveType='auto'){
  try{
    const res=await fetch(`${API_BASE}/history/save`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
...
async function saveCurrentMonth(auto=false){
  if(!window.saccoData?.today){if(!auto)alert("Data not ready!\nVisit HOMEPAGE to populate.");return;}
  ...
  if(result.success){
    ...
    if(!auto){startRetry(payload,saveType);alert(`Save queued – retrying every 30s...\nError: ${result.error}`);}
  }
}
